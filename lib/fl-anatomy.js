...
// ─── Presentation (quartering angle) ──────────────────────────────────
//
// Broadside is the reference. When the deer is angled, the heart/lung target
// stays in the same place inside the body but the ENTRY point on the near side
// must move so the bullet line passes through both lungs, and the effective
// target the stalker sees shrinks. We don't claim a precise centimetre offset
// (it depends on body width, exact angle and range, all uncertain) — we give
// the direction and a reduced effective vital zone so the "approaching the
// vital zone" warning fires sooner on an angled animal.
//
// vitalFactor: fraction of the broadside vital-zone diameter that remains an
// honest margin at this angle. Quartering-to is penalised hardest — the near
// shoulder shields the chest and a steep angle risks a single lung.
//
// Sources: Deer Initiative / BPG shot-placement guidance for angled deer
// (quartering-away → line up the off-shoulder exit; quartering-to → point of
// the near shoulder, smaller margin, prefer to wait).
export const PRESENTATIONS = Object.freeze({
  broadside: {
    label: 'Broadside',
    vitalFactor: 1.0,
    text: 'Broadside — aim behind the near foreleg, centre of the chest.',
    warning: null,
  },
  quartering_away: {
    label: 'Quartering away',
    vitalFactor: 0.85,
    text: 'Quartering away — hold further back and line the shot up to exit the off (far) shoulder, so the bullet drives forward through both lungs.',
    warning: 'A steep away-angle can carry the bullet into the paunch — take it only when you can picture the exit at the far shoulder.',
  },
  quartering_to: {
    label: 'Quartering toward',
    vitalFactor: 0.6,
    text: 'Quartering toward — aim the point of the near shoulder; it shields the vitals and the margin is smaller.',
    warning: 'A steep quartering-to shot risks a single lung or a shoulder-only hit. Prefer to wait for a broadside or quartering-away presentation.',
  },
});

export const DEFAULT_PRESENTATION = 'broadside';

/**
 * Directional guidance for a quartering presentation. Pure lookup.
 * @param {string} presentationKey — key in PRESENTATIONS
 * @returns {{ key, label, text, warning, vitalFactor }}
 */
export function getQuarteringGuidance(presentationKey) {
  const key = PRESENTATIONS[presentationKey] ? presentationKey : DEFAULT_PRESENTATION;
  const p = PRESENTATIONS[key];
  return { key, label: p.label, text: p.text, warning: p.warning, vitalFactor: p.vitalFactor };
}
 ...
 * }}
 */
export function getAnatomicalHold({ dropCm, speciesKey, sex, aimPointKey, presentation, units }) {
  // 14.01 (Scott + Aaron, by email): units 'imperial' renders every LENGTH in
  // the returned sentences in inches. The maths, the warning thresholds and
  // the returned chestDepthCm stay metric — only the words a stalker reads
  // convert, so callers keep one canonical number and choose the voice.
  const fmtLen = (cm, dp) => units === 'imperial'
    ? (cm / 2.54).toFixed(dp) + ' in'   // 1 in = 2.54 cm exactly
    : cm.toFixed(dp) + ' cm';
  ...
  // Effective vital zone shrinks on a quartering deer (see PRESENTATIONS), so
  // the ranging-error warning below fires sooner on an angled animal. Broadside
  // (or an unknown/omitted presentation) leaves it unchanged.
  const pres = PRESENTATIONS[presentation] || PRESENTATIONS[DEFAULT_PRESENTATION];
  const vital = sx.vitalZoneCm * pres.vitalFactor;
  ...
  } else if (aimFraction <= 1.05) {
    // Tightened from 1.20 to 1.05 (2026-06-09): same over-claiming pattern
    // as the 0.85 → 0.95 spine-line fix (see comment above). At 1.20, the
    // bullet on a red stag (55 cm chest) sits 11 cm above the spine — that
    // is well clear of the back, not "just above". 1.05 means the phrase
    // only fires when the bullet is within ~3 cm of the back on a red,
    // ~1.5 cm on a roe — anatomically the floor of "just above".
    reference = `Just above the back`;
  } else {
    const aboveCm = (aimFraction - 1.0) * chest;
    reference = `${fmtLen(aboveCm, 0)} above the top of the back`;
  }
  ...
  if (!onBody) {
    if (aimFraction < 0) {
      ...
      warning = 'Aim point rises above the deer\'s back — the hold is off the animal. This is a long shot for this species — consider whether range is appropriate.';
    }
  }
  ...
  if (Math.abs(dropCm) > vital * 1.5 && !warning) {
    warning = pres.vitalFactor < 1
      ? `Hold offset (${fmtLen(Math.abs(dropCm), 0)}) approaches the vital zone, which is narrower on a ${pres.label.toLowerCase()} deer — a small ranging error is costly at this angle and distance.`
      : `Hold offset (${fmtLen(Math.abs(dropCm), 0)}) approaches or exceeds the vital zone — small ranging error has a large impact at this distance.`;
  }
  ...
  if (Math.abs(dropCm) < 1.0) {
    ...
    text = `Hold ${fmtLen(dropCm, 1)} high — ${reference.toLowerCase()}`;
  } else {
    // Bullet above LoS at this range (short of zero) — hold low. Plain text;
    // see comment above. UI overlays a ↓ at render time.
    text = `Hold ${fmtLen(Math.abs(dropCm), 1)} low — ${reference.toLowerCase()}`;
  }
...
const SPECIES_IMAGE = Object.freeze({
  ...
  // Muntjac and CWD intentionally omitted from SPECIES_IMAGE — they are
  // also excluded from SPECIES_BODY (see line ~82), so the anatomy panel
  // species dropdown never offers them, so these images would never render.
  // Removed 2026-06-03 alongside dropping them from sw.js PRECACHE_URLS and
  // the build script's SKIP_FILES list (saves ~50 KB of precached SVG that
  // never reached a render path). The SVGs are kept on disk under
  // species/aimthedeer/ for archival in case a future change exposes the
  // species in the anatomy panel — see PROJECT-LOG.md 2026-06-03 for the
  // domain rationale (<80 m typical engagement + small body + outsized
  // impact of modelling error mean a visualised hold isn't earnt).
});
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="30 40 240 110" role="img" aria-label="Deer silhouette with aim point and projected impact">
  ...
  <ellipse cx="${SVG_CHEST_X}" cy="${vitalCenterY.toFixed(1)}"
           ...
           class="bx-anat-vital" fill="rgba(198,40,40,0.30)" stroke="rgba(255,80,80,0.7)" stroke-width="1"/>
  ...
  <circle cx="${SVG_CHEST_X}" cy="${desiredImpactY.toFixed(1)}" r="3.5"
          class="bx-anat-impact" fill="#7adf7a" stroke="#0e1e0a" stroke-width="1"/>
  ...
  ${crosshairInViewport ? `
  <g class="bx-anat-cross" transform="translate(${SVG_CHEST_X},${crosshairY.toFixed(1)})">
    <circle r="5" fill="none" stroke="#d8b054" stroke-width="1.5"/>
    <line x1="-8" y1="0" x2="8" y2="0" stroke="#d8b054" stroke-width="1.2"/>
    <line x1="0" y1="-8" x2="0" y2="8" stroke="#d8b054" stroke-width="1.2"/>
  </g>
  ...
  <line x1="${SVG_CHEST_X}" y1="${crosshairY.toFixed(1)}"
        ...
        stroke="#d8b054" stroke-width="0.8" stroke-dasharray="1.5,1.5" opacity="0.5"/>
  ` : ''}
</svg>
  `.trim();
}
